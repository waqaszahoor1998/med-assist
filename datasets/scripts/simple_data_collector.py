#!/usr/bin/env python3
"""
Simple Medicine Data Collection for Day 2
Creates a basic medicine database for the AI assistant
"""

import json
import csv
from pathlib import Path

def create_medicine_database():
    """Create a comprehensive medicine database for the MVP"""
    
    # Define medicine data structure
    medicines = [
        {
            'id': 1,
            'name': 'Paracetamol',
            'generic_name': 'Acetaminophen',
            'brand_names': 'Tylenol, Panadol, Calpol',
            'dosage_forms': 'tablet, capsule, liquid, suppository',
            'common_doses': '500mg, 650mg, 1000mg',
            'indications': 'pain relief, fever reduction',
            'warnings': 'liver damage with overdose, alcohol interaction',
            'max_daily_dose': '4000mg',
            'frequency': 'every 4-6 hours',
            'alternatives': 'Ibuprofen, Aspirin',
            'category': 'analgesic'
        },
        {
            'id': 2,
            'name': 'Ibuprofen',
            'generic_name': 'Ibuprofen',
            'brand_names': 'Advil, Motrin, Nurofen',
            'dosage_forms': 'tablet, capsule, liquid, gel',
            'common_doses': '200mg, 400mg, 600mg, 800mg',
            'indications': 'pain relief, inflammation, fever',
            'warnings': 'stomach irritation, kidney problems, heart risks',
            'max_daily_dose': '2400mg',
            'frequency': 'every 6-8 hours',
            'alternatives': 'Paracetamol, Aspirin, Naproxen',
            'category': 'NSAID'
        },
        {
            'id': 3,
            'name': 'Amoxicillin',
            'generic_name': 'Amoxicillin',
            'brand_names': 'Amoxil, Trimox',
            'dosage_forms': 'capsule, tablet, liquid',
            'common_doses': '250mg, 500mg, 875mg',
            'indications': 'bacterial infections, respiratory infections',
            'warnings': 'allergic reactions, diarrhea, complete course required',
            'frequency': 'every 8 hours',
            'alternatives': 'Azithromycin, Cephalexin',
            'category': 'antibiotic'
        },
        {
            'id': 4,
            'name': 'Metformin',
            'generic_name': 'Metformin',
            'brand_names': 'Glucophage, Glumetza',
            'dosage_forms': 'tablet, extended-release tablet',
            'common_doses': '500mg, 850mg, 1000mg',
            'indications': 'type 2 diabetes, blood sugar control',
            'warnings': 'lactic acidosis, kidney function monitoring',
            'frequency': 'twice daily with meals',
            'alternatives': 'Glipizide, Glyburide',
            'category': 'antidiabetic'
        },
        {
            'id': 5,
            'name': 'Lisinopril',
            'generic_name': 'Lisinopril',
            'brand_names': 'Prinivil, Zestril',
            'dosage_forms': 'tablet',
            'common_doses': '5mg, 10mg, 20mg, 40mg',
            'indications': 'high blood pressure, heart failure',
            'warnings': 'cough, low blood pressure, kidney problems',
            'frequency': 'once daily',
            'alternatives': 'Enalapril, Captopril',
            'category': 'ACE inhibitor'
        },
        {
            'id': 6,
            'name': 'Atorvastatin',
            'generic_name': 'Atorvastatin',
            'brand_names': 'Lipitor',
            'dosage_forms': 'tablet',
            'common_doses': '10mg, 20mg, 40mg, 80mg',
            'indications': 'high cholesterol, heart disease prevention',
            'warnings': 'muscle pain, liver problems',
            'frequency': 'once daily',
            'alternatives': 'Simvastatin, Rosuvastatin',
            'category': 'statin'
        },
        {
            'id': 7,
            'name': 'Omeprazole',
            'generic_name': 'Omeprazole',
            'brand_names': 'Prilosec, Losec',
            'dosage_forms': 'capsule, tablet',
            'common_doses': '20mg, 40mg',
            'indications': 'acid reflux, stomach ulcers, GERD',
            'warnings': 'bone fractures with long-term use, vitamin B12 deficiency',
            'frequency': 'once daily before meals',
            'alternatives': 'Lansoprazole, Esomeprazole',
            'category': 'PPI'
        },
        {
            'id': 8,
            'name': 'Aspirin',
            'generic_name': 'Acetylsalicylic acid',
            'brand_names': 'Bayer, Bufferin',
            'dosage_forms': 'tablet, chewable tablet',
            'common_doses': '81mg, 325mg, 500mg',
            'indications': 'pain relief, fever, heart attack prevention',
            'warnings': 'stomach bleeding, Reye syndrome in children',
            'frequency': 'every 4-6 hours or once daily for prevention',
            'alternatives': 'Paracetamol, Ibuprofen',
            'category': 'NSAID'
        },
        {
            'id': 9,
            'name': 'Azithromycin',
            'generic_name': 'Azithromycin',
            'brand_names': 'Zithromax, Z-Pak',
            'dosage_forms': 'tablet, capsule, liquid',
            'common_doses': '250mg, 500mg',
            'indications': 'bacterial infections, respiratory infections',
            'warnings': 'heart rhythm problems, hearing loss',
            'frequency': 'once daily',
            'alternatives': 'Amoxicillin, Clarithromycin',
            'category': 'antibiotic'
        },
        {
            'id': 10,
            'name': 'Loratadine',
            'generic_name': 'Loratadine',
            'brand_names': 'Claritin, Alavert',
            'dosage_forms': 'tablet, liquid, chewable tablet',
            'common_doses': '10mg',
            'indications': 'allergies, hay fever, hives',
            'warnings': 'drowsiness (rare), dry mouth',
            'frequency': 'once daily',
            'alternatives': 'Cetirizine, Fexofenadine',
            'category': 'antihistamine'
        }
    ]
    
    # Create directories
    base_dir = Path(__file__).parent.parent
    raw_dir = base_dir / "raw"
    processed_dir = base_dir / "processed"
    
    raw_dir.mkdir(exist_ok=True)
    processed_dir.mkdir(exist_ok=True)
    
    # Save as JSON
    json_file = processed_dir / "medicines_database.json"
    with open(json_file, 'w') as f:
        json.dump(medicines, f, indent=2)
    
    # Save as CSV
    csv_file = processed_dir / "medicines_database.csv"
    with open(csv_file, 'w', newline='') as f:
        if medicines:
            fieldnames = medicines[0].keys()
            writer = csv.DictWriter(f, fieldnames=fieldnames)
            writer.writeheader()
            writer.writerows(medicines)
    
    print(f"✓ Created medicine database with {len(medicines)} medicines")
    print(f"✓ JSON saved to: {json_file}")
    print(f"✓ CSV saved to: {csv_file}")
    
    return medicines

if __name__ == "__main__":
    medicines = create_medicine_database()
    print(f"\nMedicine Database Created Successfully!")
    print(f"Total medicines: {len(medicines)}")
    print(f"Categories: {set(m['category'] for m in medicines)}")
